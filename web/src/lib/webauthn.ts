...
export class WebAuthnService {
  ...
  static async register(userId: string, userName: string): Promise<Credential | null> {
    ...
    try {
      ...
      const publicKeyOptions: PublicKeyCredentialCreationOptions = {
        ...
        rp: {
          name: this.RP_NAME,
          id: this.RP_ID === 'localhost' ? 'localhost' : this.RP_ID.split('.').slice(-2).join('.'),
        },
        user: {
  ...
  static async authenticate(reason: string): Promise<boolean> {
    ...
    try {
      ...
      const options: PublicKeyCredentialRequestOptions = {
        ...
        timeout: 60000,
        userVerification: 'required',
        rpId: this.RP_ID === 'localhost' ? 'localhost' : this.RP_ID.split('.').slice(-2).join('.'),
      };
